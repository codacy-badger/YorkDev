...
//  Getting Started
  ...
  firstbot: { url: '/getting-started/your_basic_bot.html', snippet: 'In this chapter I\'ll guide you through the development of a simple bot with some useful commands. We\'ll start with the example we created in the first chapter and expand upon it.' },
...
exports.run = (client, message, [keyword]) => {
  if (guides[keyword]) {
    const details = guides[keyword];
    message.channel.send(`${details.snippet}\n**Read More**: <${baseUrl}${details.url}>`);
  } else if (keyword === '-list') {
    message.channel.send(`**❯ Pages: **${Object.keys(guides).join(', ')}`);
  } else {
